...
class KosmosConfig:
    ...
    def find_usb_path(self):
        ...
        logging.debug(
            f"Recherche clef usb lancement script : ./kosmos_find_usb.sh {USB_ROOT_PATH}"
        )
        result = subprocess.run(
            ["./kosmos_find_usb.sh", USB_ROOT_PATH], capture_output=True
        )
    ...
    def __init__(self):
        ...
        if self._usb_path != "" and os.path.isfile(self._usb_path + "/" + CONF_FILE):
            ...
            if self._cur_dir is not None and os.path.isfile(
                ...
                logging.info(
                    f"Fichier de configuration lu en local {self._cur_dir}/{CONF_FILE}"
                )
            else:
    ...
    def copy_file(self, aFileName: str) -> bool:
        ...
        if self._usb_path != "":
            result = subprocess.run(
                ["sudo", "cp", aFileName, self._usb_path], capture_output=True
            )
            if result.returncode == 0:
                return True
        return False
    ...
    def rm_file(self, aFileName: str) -> bool:
        ...
        result = subprocess.run(["rm", aFileName], capture_output=True)
        if result.returncode == 0:
            return True
        return False
    ...
    def moove_file(self, aFileName: str) -> bool:
        ...
        if self.copy_file(aFileName) is True:
            if self.rm_file(aFileName):
                logging.info(
                    f"Le fichier {aFileName} a bien été déplacé vers la clef USB."
                )
                return True
